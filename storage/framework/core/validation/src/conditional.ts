 ...
 *
 * The base validator is NOT mutated — refinements stack onto whatever
 * the refine function returns, so an app that calls `.when(...).when(...)`
 * sees both refinements compose without polluting the base instance.
 */
export function applyConditionals<V extends Validator<any>>(
  ...
  for (const record of records) {
    if (shouldApplyConditional(record, parent))
      current = record.refine(current)
  }
  return current
}
