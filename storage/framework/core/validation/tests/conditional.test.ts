...
import { applyConditionals, schema, shouldApplyConditional, withConditionals } from '../src/schema'
import { string as tsString } from '@stacksjs/ts-validation'
...
describe('schema.string().sometimes()', () => {
  test('skips rules when the key is absent from input', () => {
    ...
    // absent → no minLength enforcement, passes
...
describe('introspection: __conditionals is publicly accessible', () => {
  ...
  test('shouldApplyConditional + applyConditionals are pure', () => {
    ...
    const augmented = withConditionals(tsString())
